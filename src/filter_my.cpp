...
//除去指定角度外的角度
void Filter::out_range_points(std::vector<float>&data,std::vector<float>&theta,float theth_res)
{
...
//筛选距离过远的点
void Filter::dis_longer_filter(std::vector<float>&data,const int maxdis)
{
    for(int i=0;i<DATA_NUM;i++)
    {
        if(data[i]>maxdis)
...
//筛选距离过近的点
void Filter::dis_shorter_filter(std::vector<float>&data,const int mindis)
{
  for(int i=0;i<DATA_NUM;i++)
    {
        if(data[i]<mindis)
